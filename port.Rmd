...
r1 <- ggplot(acf_df, aes(x = Lag1, y = ACF1)) +
  geom_bar(stat = "identity", fill = "purple4", alpha = 0.7, width = 0.2) +
...
r2 <- ggplot(acf_df, aes(x = Lag2, y = ACF2)) +
  geom_bar(stat = "identity", fill = "green4", alpha = 0.7, width = 0.2) +
  geom_hline(yintercept = 0, color = "black") +
...
```{r}
#TODO: yo aca ya descartaria el modelo manual
...
h1 <- ggplot(residuos1, aes(x = .resid)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "purple4", alpha = 0.7) +
  geom_density(color = "red", size = 1) +
  labs(
    title = "Histograma de los residuos modelo automatico",
    x = "Residuos",
    y = "Densidad"
  ) +
  theme_minimal()
...
# Gráfico
g <- ggplot(emae_mensual_ts_recent, aes(x = periodo, y = emae_original)) + 
  geom_line(aes(color = "Serie Original"), size = 1) + # Serie original
  geom_line(data = as.data.frame(f1), aes(x = periodo, y = .mean, color = "Predicción Auto"), size = 1) + # Predicción automática
  geom_line(data = as.data.frame(f2), aes(x = periodo, y = .mean, color = "Predicción Manual"), size = 1) + # Predicción manual
  geom_point(data = values_to_predict_df, 
             aes(x = periodo, y = values, color = "Valores Reales"), 
             size = 2) + # Valores reales
  labs(
    title = "EMAE con predicciones y valores reales",
    y = "Índice (2004 = 100)",
    x = "",
    color = "Leyenda"
  ) +
  ...
  scale_color_manual(values = c(
    "Serie Original" = "cyan4", 
    "Predicción Auto" = "purple4", 
    "Predicción Manual" = "green4", 
    "Valores Reales" = "cyan4"
  ))
ggplotly(g)
```
...
```{r}
# Gráfico
g <- autoplot(emae_mensual_ts_recent) + 
  autolayer(f1, series = "Predicción", color = "purple4") + 
